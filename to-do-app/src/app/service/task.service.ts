...
export class TaskService {
  ...
  constructor(private http: HttpClient,
    ...
    this.http.get<Array<TaskDTO>>(`${this.API_BASE_URL}?email=${this.authService.getPrincipal()?.email}`)
  ...
  createTask(description: string): Promise<void> {
    const taskDTO = new TaskDTO(null, description, null, this.authService.getPrincipal()!.email!);
